...
export default class TaskService {
  ...
  // Private reference to MongoDB tasks collection
  #collection;

  // Private initialization connecting to MongoDB, setting up and indexing tasks collection, and loading existing data.
  async #init() {
    try {
      ...
      console.error('Failed to initialize TaskService:', err);
      throw err;
    }
  }

  // Creates a new TaskService and runs its private initialization.
  static async create() {
  ...
  async addTask(data) {
    ...
    await this.#collection.insertOne(task.toJSON());
  ...
  async updateTask(id, newName, description) {
    const task = this.getTask(id);
    ...
    if (task.name != newName) {
      ...
      await this.#collection.updateOne({ id }, { $set: { name: newName } });
    }
    // Checks if description is provided
    if (description !== undefined) {
      // Updates task with new description
      task.setDescription(description);
      await this.#collection.updateOne({ id }, { $set: { description } });
    } 
    return task;
  }
