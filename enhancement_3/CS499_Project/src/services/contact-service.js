...
export default class ContactService {
  ...
  // Private reference to MongoDB contacts collection
  #collection;

  // Private initialization connecting to MongoDB, setting up and indexing contacts collection, and loading existing data.
  async #init() {
    try {
      ...
      console.error('Failed to initialize ContactService:', err);
      throw err;
    }
  }

  // Creates a new ContactService and runs its private initialization.
  static async create() {
  ...
  async addContact(data) {
    ...
    await this.#collection.insertOne(contact.toJSON());
  ...
  async updateFirstName(id, newFirstName) {
    ...
    if (contact.firstName != newFirstName) {
      ...
      await this.#collection.updateOne({ id }, { $set: { firstName: newFirstName } });
    }
    return contact;
  }
  ...
  async updateLastName(id, newLastName) {
    ...
    if (contact.lastName != newLastName) {
      ...
      await this.#collection.updateOne({ id }, { $set: { lastName: newLastName } });
    }
    return contact;
  }
  ...
  async updatePhoneNumber(id, phoneNumber) {
    ...
    await this.#collection.updateOne({ id }, { $set: { phoneNumber } });
    return contact;
  }
  ...
  async updateAddress(id, address) {
    ...
    await this.#collection.updateOne({ id }, { $set: { address } });
    return contact;
  }
